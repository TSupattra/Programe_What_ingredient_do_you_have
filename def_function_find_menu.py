...
def find_menu(input_type,input_indre):
    ...
    #     print(len(list_row_re_nan))
        if len(list_row_re_nan) != 0:
            ...
            # print(len(list_row_re_nan))
            # print(num_total_ingredient[name_index])
    ...
    list_tt_list_locingre = []
    dict_name_food = {}
    input_cat = input_type
    for name_type in input_cat:
        for index, row in df.iterrows():
            # print(row)
            list_tt_list_locingre_temp = []
                ...
                for i in input_indre:
                    for j in df.columns:
                        ...
                        if i == row[j]:
                            ...
                            print(row['Name_Food'])
                            print(j)
                            list_tt_list_locingre_temp.append(row['Name_Food'])
                            list_tt_list_locingre_temp.append(j)
                    if score > 0 :
                        dict_name_food[row['Name_Food']] = score
    #                     print(f"{row['Type_Food']} : {row['Name_Food']}")

                list_tt_list_locingre.append(list_tt_list_locingre_temp)
    ...
    list_tt_list_locingre_set =[]
    for i in list_tt_list_locingre:
        if len(i) != 0 :
            set_i = set(i)
            list_set_i = list(set_i)
            list_set_i.sort(reverse=True)
            new_list_set_i = []
            for j in range(len(list_set_i)):
                if j !=0 :
                    tj =list_set_i[j]
                    x = tj.replace('ingredint','')
                    new_list_set_i.append(x)
                else:
                    new_list_set_i.append(list_set_i[j])

            list_tt_list_locingre_set.append(new_list_set_i)
    # print(list_tt_list_locingre_set)
...
#         print(index_find_type)
    ...
    return df_list_dict_name, list_tt_list_locingre_set 
...
def gen_column(input_type,input_indre):
    ...
    for i in list_tt_list_locingre:
        if len(i) != 0 :
            ...
            list_set_i = list(set_i)
            list_set_i.sort(reverse=True)
            list_tt_list_locingre_set.append(list_set_i)
    # print(list_tt_list_locingre_set)
...
def get_all_menu():
    ...
    for index, row in df.iterrows():
        ...
        if len(list_row_re_nan) != 0:
            name_index = list_row_re_nan[2]
            list_total_name_food.append(name_index)
